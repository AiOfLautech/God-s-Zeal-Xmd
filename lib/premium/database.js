...
const fallbackDbPath = `${dbPath}.fallback`;
...
try {
    ...
    console.warn(`[premium] Using fallback database at ${fallbackDbPath}. Original database was left untouched.`);
    db = createDatabaseConnection(fallbackDbPath);
    configureDatabase(db);
}
...
function buildKeyMaterial() {
    const base = [
        settings.botName || 'GODS ZEAL XMD',
